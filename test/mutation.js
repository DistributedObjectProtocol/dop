...
// THIS MUST BE CHANGED TO USE THE CURRENT RATHER THAN THIS HACK FOR THE OLD ONE
// THIS MUST BE CHANGED TO USE THE CURRENT RATHER THAN THIS HACK FOR THE OLD ONE
// THIS MUST BE CHANGED TO USE THE CURRENT RATHER THAN THIS HACK FOR THE OLD ONE
...
test('Object.del subobject', function(t) {
    ...
    try {
        ...
        t.equal(
            e instanceof Error,
            true,
            'This must throw an error because deep2 is not inside of a registered object anymore'
        )
    }
...
test('Same items after array mutations', function(t) {
    ...
    t.deepEqual(
        dop.getObjectPath(array.array[0]),
        [3, 'array', 0],
        'correct path'
    )
...
test('unobserve = observer.observe(...)', function(t) {
    ...
    var tounobserve = observer.observeObject(object)
    set(object, 'newprop', '12345')
    t.equal(times, 1)
    set(object, 'newprop', '12345')
    t.equal(times, 1)
...
test('mutating length of array', function(t) {
    ...
    observer.observeProperty(object.array, 'length')
