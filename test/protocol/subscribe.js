...
test('SUBSCRIBE TESTS', function(tt) {
    ...
    var clientlistening = dopClient.listen({
    ...
    var clientclient = dopClientClient.connect({
        ...
        listener: clientlistening
    })
    clientlistening.on('connect', function(node) {
        if (typeof clientClient == 'undefined') clientClient = node
    })
    ...
    test('Before onSubscribe is defined', function(t) {
        client.subscribe('DATA').catch(function(err) {
            t.equal(
                err,
                dopClient.core.error.reject_remote[1],
                'Object not found'
            )
            t.end()
        })
    ...
    test('Client subscribe synchronously', function(t) {
        ...
        client
            ...
            .then(function(obj) {
                ...
                t.deepEqual(
                    objectDataServer.object,
                    dopServer.getObjectProxy(DATA),
                    'Same object stored in server'
                )
                t.equal(
            ...
            .then(function(obj_deep) {
                t.equal(
                    obj_deep,
                    objClient.deep,
                    'DATA_DEEP is the same object'
                )
                t.deepEqual(
                    obj_deep,
                    DATA.deep,
                    'DATA_DEEP is deepEqual to original data'
                )
            ...
            .then(function(obj_deeper) {
                t.equal(
                    obj_deeper,
                    objClient.deep.deeper,
                    'DATA_DEEPER is the same object'
                )
                t.deepEqual(
                    obj_deeper,
                    DATA.deep.deeper,
                    'DATA_DEEPER is deepEqual to original data'
                )
            ...
            .then(function(obj) {
                ...
                t.deepEqual(
                    objectDataServer2.node[serverClient.token].subscriber,
                    1,
                    'Client subscriber'
                )
                t.deepEqual(
                    objectDataServer2.node[serverClient.token].owner,
                    0,
                    'Client is not owner'
                )
                t.deepEqual(
                    objectDataClient2.node[client.token].subscriber,
                    0,
                    'Server is not subscriber'
                )
                t.deepEqual(
                    objectDataClient2.node[client.token].owner,
                    2,
                    'Server is owner'
                )
            ...
            .catch(function(err) {
                t.equal(
                    err,
                    dopClient.core.error.reject_remote[1],
                    'Object not found subscription rejected'
                )
    ...
    test('Client subscribe asynchronously different objects', function(t) {
        ...
        client.subscribe().then(function(obj) {
            ...
            t.equal(
                objectClient1.hasOwnProperty('test'),
                true,
                'objectClient1 hasOwnProperty test'
            )
            t.equal(
                objectClient2.hasOwnProperty('test'),
                true,
                'objectClient2 hasOwnProperty test'
            )
            t.equal(
                objectClient3.hasOwnProperty('test'),
                true,
                'objectClient3 hasOwnProperty test'
            )
    ...
    test('Server subscribe client object', function(t) {
        ...
        serverClient.subscribe().then(function(objServer) {
            var objectDataServer =
                dopServer.data.object[dopServer.getObjectId(objServer)]
            var objectDataClient =
                dopClient.data.object[dopServer.getObjectId(objClient)]
            t.equal(
                objServer.hasOwnProperty('test'),
                true,
            ...
            t.equal(
                objectDataServer.node[serverClient.token].owner > 0,
                true,
                'Client is owner'
            )
            t.end()
    ...
    test('Server subscribe client object that is from server', function(t) {
        ...
        client.subscribe().then(function(obj) {
            ...
            serverClient.subscribe().then(function(objServer2) {
                ...
                t.equal(
                    dopServer.getObjectId(objServer2),
                    dopServer.getObjectId(objServer) + 1,
                    'Different ids'
                )
                t.equal(
                    objServer2.hasOwnProperty('test'),
                    true,
                    'obj hasOwnProperty test'
                )
                t.deepEqual(objServer, objClient)
                ...
                var objectDataServer =
                    dopServer.data.object[dopServer.getObjectId(objServer)]
                var objectDataServer2 =
                    dopServer.data.object[dopServer.getObjectId(objServer2)]
                var objectDataClient =
                    dopClient.data.object[dopServer.getObjectId(objClient)]
                t.equal(
                    objectDataServer.node[serverClient.token].subscriber,
                    1,
                    'Client is subscriber'
                )
                t.equal(
                    objectDataServer.node[serverClient.token].owner,
                    0,
                ...
                t.equal(
                    objectDataServer2.node[serverClient.token].owner > 0,
                    true,
                    'Client is owner'
                )
                t.equal(
                ...
                t.equal(
                    objectDataClient.node[client.token].owner > 0,
                    true,
                    'Server is not owner'
                )
    ...
    test('Client subscribe into object that is not register yet', function(t) {
        ...
        client
            ...
            .then(function(obj) {
                t.equal(
                    dopClient.isRegistered(objClient),
                    true,
                    'Is registered'
                )
                t.notDeepEqual(
                    objClient,
                    objClientSnap,
                    'Are not equivalent before subcribe'
                )
                t.deepEqual(
                    objClient,
                    objServer,
                    'Are equivalent with server objetc'
                )
                t.equal(
                    obj,
                    dopClient.getObjectProxy(objClient),
                    'objClient and obj are the same object'
                )
                t.end()
            })
    })
            ...
            .then(function(obj) {
                ...
                serverClient
                    ...
                    .then(function(obj2) {
                        t.equal(
                            obj2,
                            dopServer.getObjectProxy(objServer),
                            'Same object server'
                        )
                        t.deepEqual(objClient, objServer)
                        var objectDataServer =
                            dopServer.data.object[
                                dopServer.getObjectId(objServer)
                            ]
                        var objectDataClient =
                            dopClient.data.object[
                                dopServer.getObjectId(objClient)
                            ]
                        t.equal(
                            objectDataServer.node[serverClient.token]
                                .subscriber,
                            1,
                            'Client is subscriber'
                        )
        ...
        client.subscribe().then(function(obj) {
            ...
            clientclient.subscribe().then(function(obj2) {
                ...
                var objectDataServer =
                    dopServer.data.object[dopServer.getObjectId(objServer)]
                var objectDataClient =
                    dopClient.data.object[dopClient.getObjectId(objClient)]
                var objectDataClientClient =
                    dopClientClient.data.object[dopClient.getObjectId(obj2)]
                t.equal(
                    objectDataServer.node[serverClient.token].subscriber,
                    1,
                    'Client is subscriber'
                ...
                t.equal(
                    objectDataClient.node[clientClient.token].subscriber,
                    1,
                    'ClientClient is subscriber'
                )
                t.equal(
                    objectDataClient.node[clientClient.token].owner,
                    0,
                    'ClientClient is not owner'
                )
                t.equal(
                    objectDataClientClient.node[clientclient.token].subscriber,
                    0,
                    'Client is not subscriber'
                )
                t.equal(
                    objectDataClientClient.node[clientclient.token].owner > 0,
                    true,
                    'Client is owner'
                )
    ...
    test('Server <-> Client <-> ClientClient into the same object', function(t) {
        ...
        clientclient
            .subscribe()
            .into(objClientClient)
            .then(function(obj) {
                t.equal(
                    obj,
                    dopClientClient.getObjectProxy(objClientClient),
                    'Same object clientclient'
                )
                client
                    .subscribe()
                    .into(objClient)
                                ...
                                clientClient
                                    ...
                                    .then(function(obj) {
                                        ...
                                        t.deepEqual(
                                            objServer,
                                            objClient,
                                            'deepEqual to objServer objClient'
                                        )
                                        t.deepEqual(
                                            objClient,
                                            objClientClient,
                                            'deepEqual to objServer objClientClient'
                                        )
                                        ...
                                        var objectDataClientClient =
                                            dopClientClient.data.object[
                                                dopServer.getObjectId(
                                                    objClientClient
                                                )
                                            ]
                                        t.equal(
                                            objectDataServer.node[
                                                serverClient.token
                                            ].subscriber,
                                            1,
                                            'Client is subscriber'
                                        ...
                                        t.equal(
                                            objectDataClient.node[client.token]
                                                .subscriber,
                                            1,
                                            'Server is subscriber'
                                        )
                                        ...
                                        t.equal(
                                            ...
                                            true,
                                            'ClientClient is owner'
                                        )
                                        t.equal(
                                            objectDataClientClient.node[
                                                clientclient.token
                                            ].subscriber,
                                            1,
                                            'Client is subscriber'
