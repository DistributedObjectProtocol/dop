...
test('addComputed', function(t) {
    ...
    dop.set(
        object,
        'fullname',
        dop.computed(function(oldvalue) {
            t.equal(oldvalue, '...')
            runs += 1
            return get(this, 'name') + ' ' + get(this, 'surname')
        })
...
test('Deep property', function(t) {
    ...
    var object = dop.register({
        ...
        fullname: computed(function(oldvalue) {
            t.equal(oldvalue, undefined)
            return (
                get(this.subobject, 'name') +
                ' ' +
                get(this.subobject, 'surname')
            )
        })
    })
    ...
    var object = dop.register({
        ...
        fullname: computed(function(oldvalue) {
            try {
                ...
                t.equal(
                    e.toString().indexOf('TypeError') > -1,
                    true,
                    'Is a typeError'
                )
            }
...
test('Update computeds', function(t) {
    ...
    var object = dop.register({
        ...
        completedCount: computed(function() {
            return get(this, 'todos').reduce(function(sum, todo) {
                return sum + (get(todo, 'completed') ? 1 : 0)
            }, 0)
        }),
...
test('Computed must listen mutations even if property is not defined (arrays)', function(t) {
    ...
    t.equal(collector.mutations.length, 3)
    object.items.push(true)
    set(object.items, 'demo2', true)
    t.equal(object.totalItems, 2)
    t.equal(collector.mutations.length, 6)
    collector.emit()

    t.end()
})

test('Using get syntax', function(t) {
    var object = dop.register({
        items: [],
        get totalItems() {
            return get(this, 'items').length
