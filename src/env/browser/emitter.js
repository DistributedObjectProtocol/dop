...
dop.util.emitter.prototype.on = function(name, callback, once) {
    if (isFunction(callback)) {
        if (!isObject(this._events)) this._events = {}
        if (!isObject(this._events[name])) this._events[name] = []
        this._events[name].push(once === true ? [callback, true] : [callback])
    }
    return this
}
...
dop.util.emitter.prototype.emit = function(name) {
    if (isObject(this._events[name]) && this._events[name].length > 0) {
        ...
        for (i = 0; i < fun.length; i++) fun[i].apply(this, args)
    }
    return this
}
