...
dop.core.decode = function(property, value, undefineds) {
    if (typeof value == 'string') {
        ...
        if (value == dop.protocol.instructionsPatchs.infinity) return Infinity

        if (value == dop.protocol.instructionsPatchs._infinity) return -Infinity

        if (value == dop.protocol.instructionsPatchs.nan) return NaN

        if (regexpdate.exec(value)) return new Date(value)
        ...
        if (value[0] == '~')
            // https://jsperf.com/charat-vs-index/5
