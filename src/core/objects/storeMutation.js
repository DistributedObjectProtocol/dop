dop.core.storeMutation = function(mutation) {
    ...
    // Saving path_id
    path_id_parent = path_id = mutation.path_id = dop.core.getPathId(
        mutation.path
    )
    ...
    if (
        !dop.core.runInterceptors(
            paths[path_id_parent],
            'interceptors',
            mutation
        )
    )
    ...
    if (
        !dop.core.runInterceptors(paths[path_id], 'interceptors_prop', mutation)
    )
        return
...
dop.core.runInterceptors = function(interceptors, type, mutation) {
    if (
        interceptors &&
        (interceptors = interceptors[type]) &&
        ...
        for (var index = 0, total = interceptors.length; index < total; ++index)
            if (
                interceptors[index](
                    mutation,
                    dop.getObjectTarget(mutation.object)
                ) !== true
            )
                return false
