...
//////////  src/side/server/listener/socketio.js

// http://socket.io/docs/server-api/
dop.listener.socketio = function ( options, on ) {

    options.connector = options._connector; // Need it because socketio accept the option connector as parameter natively

    var socket_server = new dop.listener.socketio.api( options.httpServer, options );
    ...
    socket_server.of( options.namespace ).on('connection', function( user ){
        ...
        user.send = dop.listener.socketio.send;

        user.close = dop.listener.socketio.close;
...
dop.listener.socketio.api = require('socket.io');
dop.listener.socketio._name = 'socketio';

dop.listener.socketio.send = function( data ) {
    this.emit('message', data);
};
dop.listener.socketio.close = function( ) {
    this.disconnect();
};
...
//////////  src/side/server/listener/sockjs.js

// https://github.com/sockjs/sockjs-node
dop.listener.SockJS = function ( options, on ) {
    ...
    var socket_server = dop.listener.SockJS.api.createServer( options );
    ...
    socket_server.on('connection', function(user) {
        ...
        user.send = dop.listener.SockJS.send;
...
dop.listener.SockJS.api = require('sockjs');
dop.listener.SockJS._name = 'SockJS';

dop.listener.SockJS.send = function( data ) {
    this.write( data );
};
...
//////////  src/side/server/listener/ws.js

// https://github.com/websockets/ws
dop.listener.ws = function ( options, on ) {
    ...
    var socket_server = new dop.listener.ws.api.Server( options );
...
dop.listener.ws.api = require('ws');
dop.listener.ws._name = 'ws';
